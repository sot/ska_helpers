...
By-hand process for guiding CoPilot:

- Go to the version N-1 view of the guideline. - Select Raw View at the bottom.
...
(https://cli.github.com/). In addition you must be logged in to `gh` with
`gh auth login`.
...
def get_argparser():
    argparser = argparse.ArgumentParser(prog="ska_twiki_diff", usage=doc)
